...
let modalState = null; // { all: {ollama:string[], openai:string[]}, selected: Set<string>, filter: string }
...
function getSelectedModels() {
  try { const raw = localStorage.getItem('selected_models'); if (!raw) return []; const arr = JSON.parse(raw); return Array.isArray(arr) ? arr : []; } catch { return []; }
}
function setSelectedModels(list) { try { localStorage.setItem('selected_models', JSON.stringify(Array.from(new Set(list)))) } catch {} }
...
async function listAllModels() {
  ...
  const tasks = [];
  tasks.push((async () => { try { const r = await fetch(`${ollamaBase}/api/tags`); if (r.ok) { const d = await r.json(); results.ollama = (d.models||[]).map(m=>m.name).sort((a,b)=>a.localeCompare(b)); } } catch {} })());
  tasks.push((async () => { if (!token) return; try { const r = await fetch(`${openaiBase.replace(/\/$/, '')}/models`, { headers: { 'Authorization': `Bearer ${token}` } }); if (r.ok) { const d = await r.json(); results.openai = (d.data||[]).map(m=>m.id).sort((a,b)=>a.localeCompare(b)); } } catch {} })());
...
function renderCombined(all, filterText = "") {
  ...
  const renderSection = (label, items) => {
    const hdr = document.createElement('div'); hdr.className = 'selected-title'; hdr.textContent = label; els.modelsList.appendChild(hdr);
    const list = apply(items || []);
    if (!list.length) { const none = document.createElement('div'); none.className='model-item'; none.textContent='No models'; els.modelsList.appendChild(none); return; }
    list.forEach(name => {
      const item = document.createElement('label'); item.className='model-item';
      const cb = document.createElement('input'); cb.type='checkbox'; cb.value=name; cb.checked = modalState && modalState.selected ? modalState.selected.has(name) : false;
      cb.addEventListener('change', () => { if (!modalState) return; if (cb.checked) modalState.selected.add(name); else modalState.selected.delete(name); updateSelectedModelsView(); });
      const span=document.createElement('span'); span.textContent=name; item.append(cb, span); els.modelsList.appendChild(item);
    });
  };
...
function updateSelectedModelsView() {
  ...
  arr.forEach(name => {
    ...
    x.addEventListener('click', () => { if (!modalState) return; modalState.selected.delete(name); updateSelectedModelsView(); const cb = els.modelsList.querySelector(`input[type=\\"checkbox\\"][value=\\"${CSS.escape(name)}\\"]`); if (cb) cb.checked = false; });
    pill.append(txt, x);
...
async function openModels() {
  ...
  if (els.modelsFilter) els.modelsFilter.value = "";
  try {
...
// provider-specific translation below
...
async function translateOnceOllama(model, from, to, text, onPartial) {
  ...
  const res = await fetch(`${base}/api/generate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model, prompt, stream: true }) });
  if (!res.ok || !res.body) throw new Error(`Model ${model} failed: HTTP ${res.status}`);
  const reader = res.body.getReader(); const decoder = new TextDecoder(); let buffer = ""; let fullText = "";
  try {
    while (true) {
      ...
      let idx; while ((idx = buffer.indexOf("\n")) !== -1) {
        ...
        try {
          ...
          if (obj.response) { fullText += obj.response; if (typeof onPartial === "function") onPartial(fullText); }
          if (obj.error) throw new Error(obj.error);
          if (obj.done) break;
        } catch (e) { if (!(e instanceof SyntaxError)) throw e; buffer = line + "\n" + buffer; break; }
      }
    }
...
async function translateOnceOpenAI(model, from, to, text, onPartial) {
  const base = getOpenAiBaseUrl(); const token = getOpenAiToken(); if (!token) throw new Error('Missing OpenAI token');
  const prompt = `Translate the following text from ${from} to ${to}. Preserve meaning, lists and punctuation. Return only the translated text.\n\n${text}`;
  const url = `${base.replace(/\/$/, '')}/chat/completions`;
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ model, stream: true, messages: [ { role: 'system', content: 'You are a helpful translation engine.' }, { role: 'user', content: prompt } ] }) });
  if (!res.ok || !res.body) throw new Error(`Model ${model} failed: HTTP ${res.status}`);
  const reader = res.body.getReader(); const decoder = new TextDecoder(); let buffer = ''; let fullText = '';
  try {
    while (true) {
      ...
      let idx; while ((idx = buffer.indexOf('\n\n')) !== -1) {
        ...
        for (const ln of lines) {
          ...
          const data = m[1].trim(); if (data === '[DONE]') { buffer = ''; break; }
          try { const obj = JSON.parse(data); const delta = obj.choices && obj.choices[0] && obj.choices[0].delta && obj.choices[0].delta.content; if (delta) { fullText += delta; if (typeof onPartial === 'function') onPartial(fullText); } const err = obj.error && (obj.error.message || obj.error); if (err) throw new Error(err); } catch {}
        }
      }
    }
  } finally { try { reader.releaseLock(); } catch {} }
...
async function performTranslate() {
  ...
  const runGroup = async (label, list, fn) => {
    ...
    const sep = document.createElement('div'); sep.className = 'result-separator'; sep.textContent = label; els.results.appendChild(sep);
